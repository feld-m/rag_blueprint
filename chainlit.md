...
FELD M GmbH, located at Waltherstraße 23, 80337 Munich, Germany, and available at contact@feld-m.de , is responsible for data processing in accordance with the [General Data Protection Regulation.](https://dsgvo-gesetz.de/art-4-dsgvo/)
...
_(Note: As this is a demonstration system on our own servers, no external service provider is involved in data processing. As the operator, we alone are the responsible party)._
...
Your data will **not be passed on to third parties**. We do not use any external service providers to operate the chatbot; the system runs exclusively on our own servers. Your personal data **will not be transferred** to third parties or to countries outside the EU/EEA. The Bundestag data on which the AI model was trained is also not passed on to third parties - it is only used for the internal operation of the AI system.
...
- **Server log data (incl. IP addresses):** These are only stored for a short period of time. IP addresses in our technical logs are **automatically deleted after [e.g. 7 days]**. Longer storage only takes place if a security incident (e.g. an attack) needs to be analyzed. In such cases, the relevant log extracts are stored until the incident has been clarified and then deleted.

- **Chat entries:** We may save the content of the chats (questions and answers given by the AI) for testing and improvement purposes. Such chat logs are evaluated **pseudonymously**, i.e. without direct personal reference. We try to avoid or remove specific personal content in the requests wherever possible. The raw data of the chat entries are either deleted after [e.g. 30 days] at the latest or made unrecognizable by anonymization. We may retain aggregated or anonymized findings (without personal reference) for longer, as they no longer fall under the GDPR.
