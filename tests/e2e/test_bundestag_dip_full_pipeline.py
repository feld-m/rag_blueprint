"""
End-to-End Test: Bundestag DIP API Full Extraction Pipeline

This test validates the complete RAG pipeline for Bundestag DIP API integration:
...
class TestBundestagDIPFullPipeline:
    """
    E2E test suite for Bundestag DIP API integration.

    Uses real infrastructure:
    - PGVector database (via Docker)
    - HuggingFace embedding model (multilingual-e5-small)
    - DIP API (public endpoint)
    """

    @pytest.fixture(scope="class")
    def embedding_config(self) -> EmbeddingConfiguration:
        """
        Load embedding configuration for Bundestag DIP e2e tests.

        This fixture temporarily copies the Bundestag DIP configuration
        to configuration.test.json, loads it, then restores the original.

        Returns:
            EmbeddingConfiguration with Bundestag DIP datasource, PGVector store, and HF embeddings
        """
        import shutil
        ...
        dip_config = Path(
            "configurations/configuration.test-bundestag-dip.json"
        )
        ...
        try:
            # Copy Bundestag DIP config to test.json
            shutil.copy(dip_config, test_config)
    ...
    async def test_full_extraction_embedding_pipeline(
        ...
        """
        Test complete pipeline: DIP API → Extraction → Embedding → PGVector.

        This test validates:
        1. EmbeddingOrchestrator correctly orchestrates the full pipeline
        2. DIP API client fetches protocol documents
        ...
        - Limited document count (export_limit=3) for speed
        ...
        # export_limit=1 protocol, split into multiple chunks
        # Expect at least 5 chunks (protocols are large, typically 50-200 chunks each)
    ...
    async def test_document_metadata_preservation(
        ...
        - DIP-specific metadata (protocol_number, legislature_period, etc.)
        ...
        assert (
            metadata["source_client"] == "dip"
        ), f"Expected source_client='dip', got '{metadata['source_client']}'"

        assert "document_type" in metadata, "Missing 'document_type' metadata"
        assert (
            metadata["document_type"] == "protocol"
        ), f"Expected document_type='protocol', got '{metadata['document_type']}'"

        # DIP-specific metadata
        assert (
            "document_number" in metadata
        ), "Missing 'document_number' metadata"
        assert (
            "legislature_period" in metadata
        ), "Missing 'legislature_period' metadata"
        # legislature_period can be int or str depending on storage
        assert (
            str(metadata["legislature_period"]) == "21"
        ), f"Expected legislature_period='21', got '{metadata['legislature_period']}'"
...
if __name__ == "__main__":
    ...
    $ python tests/e2e/test_bundestag_dip_full_pipeline.py

    Or via pytest:

    $ pytest tests/e2e/test_bundestag_dip_full_pipeline.py -v
    """
